...
function TodoList () {
  ...
  useEffect( () => {
    // getメソッドで引数内のURLでアクセスする => railsのルーティングが動く
    // indexアクションが動く
  ...
  const removeAllTodos = () => {
    ...
    if (sure) {
      // deleteメソッドで引数内のURLでアクセスする => railsのルーティングが動く
      // destroy_allアクションが動く
  ...
  /* 
    todoを更新する関数
      ...
      引数:
        index: 何番目のtodoが更新されたかを表す
        val: 更新しようとしているレコードそのもの
  */
  const updateIsCompleted = (index, val) => {
    ...
    //レコードの表示の更新
    .then(resp => {
      const newTodos = [...todos]　//スプレッド構文で展開する?????????
